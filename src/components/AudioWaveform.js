...
const AudioWaveform = () => {
	...
	const handleDownload = () => {
		if (wavesurferObj && fileURL) {
			const fileName = 'audio_waveform.wav'; // Change the filename as needed
			const audioData = wavesurferObj.exportPCM(); // Get PCM data directly from Wavesurfer
			const blob = new Blob([audioData], { type: 'audio/wav' }); // Create Blob from PCM data
			
			// Create a link element
			const link = document.createElement('a');
			link.href = URL.createObjectURL(blob); // Set the URL to the Blob
